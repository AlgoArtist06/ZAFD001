// The tiny offline Source of Truth slice shared across domain tests -
// a verbatim port of tests/conftest.py so the whole gold set resolves
// against this fixture exactly as it does in Python.
import { type Chunk } from "../../convex/lib/models";
import { chunkAct } from "../../scripts/lib/chunker";
import { parseAct } from "../../scripts/lib/parser";

const CRIMINAL = `ACT_ID: bns
...
const CONSTITUTION = `ACT_ID: constitution
...
const GOVERNANCE = `ACT_ID: rti_act
...
const PROTECTION = `ACT_ID: dv_act
...
const WORKPLACE = `ACT_ID: posh_act
...
const SOURCES = [CRIMINAL, CONSUMER, CONSTITUTION, IP, CYBER, TRANSPORT, GOVERNANCE, PROTECTION, WORKPLACE];

export function buildCorpus(): Chunk[] {
  return SOURCES.flatMap((source) => chunkAct(parseAct(source), 30));
}
