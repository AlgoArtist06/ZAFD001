...
- :class:`Cipher` - the encryption-at-rest seam. Offline it is a deterministic,
  ...
  plaintext; production swaps :class:`~rag.infrastructure.persistence.FernetCipher`
  (authenticated AES, keyed from the environment) behind the same ``encrypt`` /
  ``decrypt`` contract, and the composition root refuses to persist with this
  offline stand-in outside development.
...
class Cipher:
    ...
    This is the offline, dependency-free stand-in only: its stored form is not
    plaintext, but XOR with a fixed key is not real encryption. Production uses
    :class:`~rag.infrastructure.persistence.FernetCipher`, and the composition
    root fails fast rather than persist with this stand-in outside development.
    """
